...
The release pipeline treats the browser VSIX and source archive as supply-chain artifacts, not just build outputs.
...
- the release tag must match the root package version.
...
- the browser extension is bundled and packaged with exact-version development dependencies declared in the root `package.json`; VS Code proposal declarations are committed under `types/vscode-proposed`, and release-contract tests keep their set aligned with the pinned 1.138.0 host and enabled proposals.
- VSIX contents are governed by `apps/demo-web/.vscodeignore`, and the exact locally produced package is unpacked and smoke-tested under Code-OSS Web.
- the exact unpacked VSIX is loaded in Code-OSS Web for a smoke test.
- SHA-256 checksums cover the VSIX, source archive, SBOM and tracked third-party notices.
- provenance attestations use GitHub OIDC via `actions/attest`.
- a validated CycloneDX JSON SBOM is generated and associated with the VSIX.
...
Release consumers can verify integrity and provenance with the release's `SHA256SUMS` and GitHub attestation:
...
1. builds the framework and browser bundle;
2. packages `remotish-demo.vsix`;
3. unpacks that exact locally produced VSIX into a smoke-test copy;
4. injects the test bundle only into the unpacked smoke-test copy;
5. runs activation, command-registration and virtual-filesystem behavior under Code-OSS Web.

The packaged file set is controlled by `apps/demo-web/.vscodeignore`; `vsce ls --no-dependencies` is the standard way to inspect that set when changing packaging rules. Release-contract tests pin the ignore rules that keep source, test bundles, TypeScript build state, and `node_modules` out of the VSIX.

Tests are injected only into the unpacked smoke-test copy, not into the release VSIX.

## Proposed API distribution constraint

The demo intentionally uses `scmActionButton`, `scmHistoryProvider`, and `timeline`. The release is therefore aimed at the controlled Code-OSS distribution described in [Code-OSS integration](code-oss-integration.md), not an ordinary Marketplace-compatible extension unless those proposal dependencies are removed or otherwise made compliant.
...
Remotish source uses the `0BSD` license. The current workspace has no third-party production npm dependencies: production edges are only internal `@remotish/*` workspace packages. Release-contract tests enforce that invariant, and the tracked `THIRD_PARTY_NOTICES.md` is copied into release artifacts. If an external runtime dependency is introduced, the contract test fails until dependency licensing/notices handling is made explicit and the notice is updated.
